...
import { PESD_CATEGORIES, TEIQUE_REVERSE_ITEMS, TEIQUE_SUBSCALES, MAIA_REVERSE_ITEMS, MAIA_SUBSCALES, PASSION_SUBSCALES } from '../constants';
...
export const ResultsTab: React.FC<ResultsTabProps> = ({ profile, data, previousData }) => {
    ...
    const downloadCSV = () => {
        ...
        if (data.teique && data.teique.length > 0) addScoresToCsv(results.teiqueScores);
        if (data.maia && data.maia.length > 0) addScoresToCsv(results.maiaScores);
        if (data.passion && data.passion.length > 0) addScoresToCsv(results.passionScores);
